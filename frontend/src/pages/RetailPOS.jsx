  ...
  const CartItems = () => (
    cart.length === 0 ? (
      ...
      <div className="space-y-2">
        {cart.map((c) => (
          <div key={c.product.id} className="flex items-start gap-2 p-3 rounded-xl bg-gray-50 border border-gray-100">
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-gray-800 leading-tight">{c.product.name}</p>
              <p className="text-xs text-gray-500 mt-0.5">{inr(c.price)} × {c.qty} = <span className="font-bold text-gray-800">{inr(c.qty * c.price)}</span></p>
            </div>
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex rounded-lg overflow-hidden border border-gray-200 shrink-0">
          <button
            ...
            className={`px-2.5 py-1.5 text-xs font-semibold transition-colors ${discount.type === "flat" ? "bg-indigo-600 text-white" : "bg-white text-gray-600 hover:bg-gray-50"}`}
          >₹</button>
          <button
            onClick={() => setDiscount(d => ({ ...d, type: "pct" }))}
            className={`px-2.5 py-1.5 text-xs font-semibold transition-colors ${discount.type === "pct" ? "bg-indigo-600 text-white" : "bg-white text-gray-600 hover:bg-gray-50"}`}
          >%</button>
        </div>
        <Input
          type="number"
        ...
        <div className="px-4 pt-3 pb-2 shrink-0">
          <div className="relative">
            ...
            <Input
              ...
              className="pl-10 pr-10 h-11 text-base border-indigo-200 focus:border-indigo-500"
              autoFocus
            />
            {search && (
              <button
              ...
              {filteredProducts.map(product => {
                ...
                return (
                  ...
                  >
                    ...
                    {product.image_b64 ? (
                      <img
                        src={product.image_b64.startsWith("data:") ? product.image_b64 : `data:image/jpeg;base64,${product.image_b64}`}
                        alt={product.name}
      ...
      <div className={`flex flex-col h-full bg-white w-full md:w-[40%]
        ...
        <div className="hidden md:flex items-center justify-between px-4 py-3 border-b border-gray-200 shrink-0">
          ...
          <div className="flex items-center gap-3 text-sm">
            <span className="text-gray-500">
              Today: <span className="font-bold text-gray-800">{sessionSales}</span>
            </span>
            {cart.length > 0 && (
        ...
        <div className="md:hidden flex items-center justify-between px-4 py-3 border-b border-gray-200 bg-white shrink-0">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-gray-800">Cart</span>
            {cartCount > 0 && (
              <Badge className="bg-indigo-600 text-white px-2">{cartCount} items</Badge>
            )}
          </div>
          {cart.length > 0 && (
        ...
        {/* cart footer with customer, discount, totals, bill button */}
        <CartFooter />
      </div>

      {/* ══════════ MOBILE BOTTOM TAB BAR ══════════ */}
      <div className="md:hidden fixed bottom-0 inset-x-0 bg-white border-t border-gray-200 flex z-40 safe-area-inset-bottom">
        <button
        ...
        >
          ...
          <span className="text-[10px] font-semibold">
            {cart.length > 0 ? `Cart • ${inr(totals.grandTotal)}` : "Cart"}
          </span>
        </button>
      </div>
